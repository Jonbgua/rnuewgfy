...
class DDIMSchedulerExtended(DDIMScheduler):
  ...
  def _get_variance_logprob(self, timestep, prev_timestep):
    # Ensure inputs are on CPU to match self.alphas_cumprod
    timestep = timestep.to('cpu') if hasattr(timestep, 'to') else timestep
    prev_timestep = prev_timestep.to('cpu') if hasattr(prev_timestep, 'to') else prev_timestep

    # Get device for output tensors from the timestep if it's a tensor
    device = timestep.device if hasattr(timestep, 'device') else 'cpu'

    alpha_prod_t = self.alphas_cumprod[timestep].to(device)
    mask_a = (prev_timestep >= 0).int().to(device)
    mask_b = 1 - mask_a
    alpha_prod_t_prev = (
        self.alphas_cumprod[prev_timestep].to(device) * mask_a
        + self.final_alpha_cumprod.to(device) * mask_b
    )
    ...
    # Return without specifying device, as it will be moved to the appropriate device in calling methods
    return variance
  ...
  def step_logprob(
    ...
    # Move timestep to CPU to match the device of self.alphas_cumprod
    timestep_cpu = timestep.to('cpu')
    prev_timestep_cpu = prev_timestep.to('cpu')

    alpha_prod_t = self.alphas_cumprod[timestep_cpu].to(timestep.device)
    mask_a = (prev_timestep >= 0).int().to(timestep.device)
    mask_b = 1 - mask_a
    alpha_prod_t_prev = (
        self.alphas_cumprod[prev_timestep_cpu].to(timestep.device) * mask_a
    ...
    variance = self._get_variance_logprob(timestep_cpu, prev_timestep_cpu).to(
        device=timestep.device, dtype=sample.dtype
    )
